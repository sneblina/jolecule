...
class SoupController {
    ...
    toggleResidueNeighbors () {
        ...
        if (nSelected === 0) {
            console.log(
                'Controller.toggleResidueNeighbors none selected: use centre of rotation'
            )
    ...
    deleteStructure (iStructure) {
        ...
        function resolveView (view) {
            ...
            _.remove(view.selected, iRes =>
                isInInterval(iRes, iResStart, iResEnd)
            )
            view.selected = _.map(view.selected, iRes =>
                patchInterval(iRes, iResStart, iResEnd)
            )

            _.remove(view.labels, l =>
                isInInterval(l.iAtom, iAtomStart, iAtomEnd)
            )
